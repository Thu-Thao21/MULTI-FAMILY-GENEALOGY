...
export async function buildErgoTreeFromDB(
  ...
  const convertNode = (m: Member, isFocus: boolean = false): ErgoTreeNode => {
    // Find potential spouse (female with same generation / father of children)
    const spouses: SpouseNode[] = [];
    if (m.gender === 'male') {
      // Find female members in same family or generation 1
  ...
  const buildChildrenRecursive = (parentId: string): ErgoTreeNode[] => {
    ...
    // If fatherId/motherId not populated explicitly, infer by generation and branch
  ...
  // Identify root ancestors (Generation 1 or roots without parents)

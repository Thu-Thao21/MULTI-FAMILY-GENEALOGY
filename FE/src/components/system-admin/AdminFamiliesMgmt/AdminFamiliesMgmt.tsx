...
const INITIAL_FAMILIES: FamilyItem[] = [];
...
export const AdminFamiliesMgmt: React.FC = () => {
  ...
  const fetchFamilies = async () => {
    try {
      ...
      console.warn('Fetch families failed, using mock data:', err);
    }
  };
  ...
  const handleDeleteFamily = (fam: FamilyItem) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa dòng họ "${fam.name}"?`)) {
      ...
      toast.success('Đã xóa dòng họ', `Dòng họ "${fam.name}" đã được xóa khỏi hệ thống.`);
    }
  };
  ...
  const handleSaveFamily = (e: React.FormEvent) => {
    ...
    if (editingFamily) {
      toast.success('Cập nhật thành công!', `Dòng họ "${formData.name}" đã được cập nhật.`);
    } else {
      toast.success('Thêm mới thành công!', `Dòng họ "${formData.name}" đã được thêm vào hệ thống.`);
    }
  };

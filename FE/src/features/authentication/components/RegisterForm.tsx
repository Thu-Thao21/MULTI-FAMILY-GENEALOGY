    ...
    >
      ...
      <div style={{ textAlign: 'center', marginBottom: '24px', position: 'relative', zIndex: 1 }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '10px', marginBottom: '18px' }}>
          ...
          <span style={{ color: '#2563eb', fontWeight: 700, letterSpacing: '0.18em', fontSize: '12px' }}>BẮT ĐẦU NGAY</span>
        </div>
        <h1 style={{ margin: 0, color: '#0f172a', fontSize: '32px', fontWeight: 800, letterSpacing: '0.3px' }}>Chào mừng bạn đến với Gia Phả Liên Họ!</h1>
        <p style={{ margin: '16px 0 0', color: '#475569', lineHeight: 1.75 }}>Tạo tài khoản để bắt đầu kết nối với gia đình và dòng họ.</p>
      </div>

      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <div>
          <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, color: '#334155' }}>Username</label>
          <input type="text" name="username" value={formData.username} onChange={handleChange} placeholder="vd: nguyen_van_a" required style={{ width: '100%', padding: '14px 16px', borderRadius: '16px', border: '1px solid rgba(148, 163, 184, 0.35)', background: '#f8fafc', boxShadow: 'inset 0 1px 2px rgba(15,23,42,0.05)', outline: 'none', transition: 'border-color 0.2s, box-shadow 0.2s' }} />
        </div>
        ...
        <div>
          ...
          <input type="text" name="displayName" value={formData.displayName} onChange={handleChange} placeholder="Nguyễn Văn A" style={{ width: '100%', padding: '14px 16px', borderRadius: '16px', border: '1px solid rgba(148, 163, 184, 0.35)', background: '#f8fafc', boxShadow: 'inset 0 1px 2px rgba(15,23,42,0.05)', outline: 'none', transition: 'border-color 0.2s, box-shadow 0.2s' }} />
        </div>

        <div>
          <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, color: '#334155' }}>Mật khẩu</label>
          <input type="password" name="password" value={formData.password} onChange={handleChange} placeholder="Ít nhất 8 ký tự" required style={{ width: '100%', padding: '14px 16px', borderRadius: '16px', border: '1px solid rgba(148, 163, 184, 0.35)', background: '#f8fafc', boxShadow: 'inset 0 1px 2px rgba(15,23,42,0.05)', outline: 'none', transition: 'border-color 0.2s, box-shadow 0.2s' }} />
        </div>

        <div>
          <label style={{ display: 'block', marginBottom: '6px', fontWeight: 600, color: '#334155' }}>Xác nhận mật khẩu</label>
          <input type="password" name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} placeholder="Nhập lại mật khẩu" required style={{ width: '100%', padding: '14px 16px', borderRadius: '16px', border: '1px solid rgba(148, 163, 184, 0.35)', background: '#f8fafc', boxShadow: 'inset 0 1px 2px rgba(15,23,42,0.05)', outline: 'none', transition: 'border-color 0.2s, box-shadow 0.2s' }} />
        </div>
        ...
        <p style={{ margin: '10px 0 0', color: '#64748b', fontSize: '13px', lineHeight: 1.6 }}>Bắt đầu hành trình gia phả với một tài khoản an toàn và đơn giản.</p>
